...
export const PricingForm = ({ sizes, setSizes, unit }) => {
  ...
  const availableSizes = allSizes
    .filter((size) => !sizes.some((selectedSize) => selectedSize.size == size))
    .map((size) => ({ value: size }));

  const addSize = () => {
    setSizes((sizes) => [
      ...sizes,
      { size: availableSizes[0].value, price: null, stock: null },
    ]);
  };
    ...
    <>
        ...
        <div className="flex gap-2" key={size}>
          <div className="grid flex-grow grid-cols-3 gap-2">
            <FormItem label="Size">
              <Select
                value={size}
                onChange={(newVal) => updateSizeValue(size, newVal)}
                className="rounded"
                placeholder="Select size"
                size="large"
                options={[{ value: size }, ...availableSizes]}
              />
            </FormItem>
            <FormItem
              rules={required()}
              onChange={(e) => updatePrice(size, e)}
              type="number"
              label="Price"
              placeholder="(e.g. ₱20)"
              inputProps={{ prefix: "₱", suffix: `/${unit}`, min: 1 }}
            />
            <FormItem
              rules={required()}
              onChange={(e) => updateStock(size, e)}
              tooltip="Number of available commodities"
              type="number"
              label="Stocks"
              inputProps={{ suffix: `${unit}`, min: 1 }}
            />
          </div>
          {sizes.length > 1 && (
      ...
      {availableSizes.length > 0 && (
        <div className="flex justify-end">
          <Button onClick={addSize} icon={<PlusOutlined />}>
            Add Size
          </Button>
        </div>
      )}
